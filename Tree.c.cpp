...
bool Tree::Check()
{
    ...
    while (!qCheck.empty())
    {
        ...
        if (qCheck.front().length() > 2)
        {
            if (qCheck.front()[0] == '<' && qCheck.front()[1] != '?' && qCheck.front()[1] != '!' && qCheck.front()[1] != '/')
            {
                found = qCheck.front().find(">", 1);
                tempFound = qCheck.front().find(" ", 1);
                if (found > tempFound)
                {
                    found = tempFound;
                }
                tagName = qCheck.front().substr(1, found - 1);

                st.push(make_pair(tagName, count));
                qCheck.pop();
                count++;
            }
            else if (qCheck.front()[0] == '<' && qCheck.front()[1] == '/')
            {
                found = qCheck.front().find(">", 1);
                tagName = qCheck.front().substr(2, found - 2);
                if (st.top().first == tagName)
                {
                    st.pop();
                    qCheck.pop();
                    count++;
                ...
                {
                    errorPostion.push_back(st.top().second);
                    st.pop();
                    getError = false;
                }
    ...
    sort(errorPostion.begin(), errorPostion.end());
    errorFound = !getError;
    return getError;
}
void Tree::copyTree()
...
void Tree::printErrors(string out)
{
    if (errorFound)
    {
        reformatForChecking();
        ...
        while (!qCheck.empty())
        {
            if (errorPostion.size() > errorsLines)
            {
                if (countLines == errorPostion[errorsLines])
                {
                    out_file << qCheck.front() + "\t\t\t *****ERROR*****" << endl;
                    qCheck.pop();
                    errorsLines++;
                }
                else
                {
                    out_file << qCheck.front() << endl;
                    qCheck.pop();
                }
            }
            else
